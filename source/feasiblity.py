...
def check_with_timewindow(route_pool,instance,Dis_List,Time_Window):
    ...
    for index,route in enumerate(route_pool):
        ...
        dp[0][Battery_Capacity][Battery_Capacity][2] = 0 # 最早到达时间
        dp[0][Battery_Capacity][Battery_Capacity][3] = 0 # 最晚离开时间
        ...
        for i in range(1,N):
            ...
            time_L = Time_Window[index][i][0] + instance['s'][route[i]] # 最早离开
            time_R = Time_Window[index][i][1] + instance['s'][route[i]] # 最晚离开时间
                    ...
                    for k in range(1,Battery_Capacity + 1):
                        if(k <= j + charge):
                            if(dp[i][j][0][0] > dp[i - 1][j + charge][k][0] and dp[i - 1][j + charge][k][3] + time + instance['s'][route[i]] < time_R):
                                ...
                                dp[i][j][0][2] = dp[i - 1][j + charge][k][3] + time # 最早到达
                                dp[i][j][0][3] = max(dp[i][j][0][2],instance['tl'][route[i]]) + instance['s'][route[i]] # 最早离开
            ...
            for j in range(0,Battery_Capacity + 1):
                for k in range(1, j + 1):
                    ...
                    if(dp[i][j][k][2] < instance['tl'][route[i]]): # 如果最早到达时间小于最早服务时间，可以先开始充电
